...
本项目为 `XXXreader` 模块，位于 `project/modules/XXXreader/`，是 XXX 解析和处理的核心库。
...
| 模块 | 路径 | 职责 |
|------|------|------|
| 解析器 A | `core/parser_a/` | 解析输入格式 A，提取结构化数据 |
| 解析器 B | `core/parser_b/` | 解析输入格式 B，支持多种变体 |
| 解析器 C | `core/parser_c/` | 统一的数据读写接口，支持多后端 |
| 格式转换器 | `core/converter/` | 格式转换和数据映射 |
...
| 目录 | 说明 |
|------|------|
| `build_output/` | 构建产物和中间文件，包含编译生成的 .obj 文件和日志 |
| `third_party_libs/` | 预编译的第三方库和公共依赖 |

> ⚠️ **注意**：`release/` 目录是所有构建命令的工作目录，必须在此目录下执行构建命令。

### 3.3 构建规范

**构建工具**：本项目基于 CMake 构建规则，使用项目自定义构建工具进行构建。
...
# 构建指定目标
build-tool build -t <target_name>

# 示例：构建 myproject 模块
build-tool build -t myproject

# 构建多个目标
build-tool build -t target1 -t target2
```

> ⚠️ **重要**：所有构建命令必须在 `release` 目录下执行。
...
项目构建系统提供了统一的第三方库管理机制，通过预定义变量和宏简化第三方库的集成。

**核心变量**：

| 变量名 | 说明 |
|--------|------|
| `PROJECT_THIRD_DEFAULT_INCLUDE_DIR` | 第三方库的默认头文件目录 |

**使用预编译的第三方库**：

```cmake
project_package(my_library STATIC)
    project_add_sources(
        src/my_source.cpp
    )
    project_include_directories(
        ...
        # 使用项目提供的第三方库头文件
        ${PROJECT_THIRD_DEFAULT_INCLUDE_DIR}/custom_tinyxml2
    )
    # 链接项目提供的第三方库（平台条件语法）
    project_use_packages(WIN(tinyxml2-custom))
project_end_package()
```
...
```cmake
# project_use_packages 支持平台条件
project_use_packages(WIN(lib_name))      # 仅 Windows 平台
project_use_packages(DARWIN(lib_name))   # 仅 macOS 平台
project_use_packages(LINUX(lib_name))    # 仅 Linux 平台
project_use_packages(lib_name)           # 所有平台
```
...
| 宏名称 | 说明 | 示例 |
|--------|------|------|
| `project_package(name TYPE)` | 定义包（STATIC/SHARED/EXECUTABLE） | `project_package(mylib STATIC)` |
| `project_end_package()` | 结束包定义 | - |
| `project_add_sources(...)` | 添加源文件 | `project_add_sources(src/main.cpp)` |
| `project_include_directories(...)` | 添加头文件路径 | `project_include_directories(include/)` |
| `project_link_packages(...)` | 链接内部包 | `project_link_packages(myproject_json)` |
| `project_use_packages(...)` | 使用外部依赖/项目提供的第三方库 | `project_use_packages(WIN(tinyxml2-custom))` |
| `project_add_definitions(...)` | 添加宏定义 | `project_add_definitions(MY_MACRO)` |
| `project_test_package(...)` | 定义测试包 | `project_test_package(test GTest::gmock_main CONSOLE)` |
| `project_add_subdirectory_on_test(...)` | 测试时才添加子目录 | `project_add_subdirectory_on_test(tests)` |

**Header-only 库 vs 预编译库**：

```cmake
# 方式一：Header-only 库（如 RapidJSON）
# 只需添加头文件路径，不需要链接
set(RAPIDJSON_INCLUDE_DIR "${CMAKE_CURRENT_SOURCE_DIR}/third_party/rapidjson/include")
project_include_directories(${RAPIDJSON_INCLUDE_DIR})

# 方式二：预编译库（如 tinyxml2）
# 需要同时添加头文件路径和链接库
project_include_directories(${PROJECT_THIRD_DEFAULT_INCLUDE_DIR}/custom_tinyxml2)
project_use_packages(WIN(tinyxml2-custom))
```

**最佳实践**：

1. **优先使用项目提供的第三方库**：保持与构建系统的一致性
2. **Header-only 库可自行管理**：放在 `third_party/` 目录，通过 `project_include_directories` 引用
3. **需要编译的第三方库**：优先使用预编译版本，避免 ABI 兼容性问题4. **平台差异处理**：使用 `WIN()`、`DARWIN()` 等条件宏处理平台差异
...
#define MYPROJECT_LOG_DEBUG(msg) ...
```
...
```cpp
// 类前缀
class CMYPROJECT_DataParser;      // C 前缀：具体类
class IMYPROJECT_DataParser;      // I 前缀：接口类
struct MYPROJECT_BoundingBox;    // 无前缀：结构体

// 枚举前缀
enum class EMYPROJECT_StyleSource;  // E 前缀：枚举

// 智能指针类型别名
using MYPROJECT_BoundingBoxSP = std::shared_ptr<MYPROJECT_BoundingBox>;
using MYPROJECT_DataParserUP = std::unique_ptr<IMYPROJECT_DataParser>;
```
...
// 5. 项目内部头文件
#include "core/parser/data_parser.h"
...
// 优先使用前向声明减少编译依赖
// 头文件中：
class DataParser;  // 前向声明
struct BoundingBox;

class MyClass {
    std::unique_ptr<DataParser> m_parser;  // 只需前向声明
};
...
// 运行时检查：检查可恢复的错误
if (index >= size) {
    MYPROJECT_LOG_ERROR("Index out of bounds");
    return std::nullopt;
}
 ...
 * 
 * @example
 * auto parser = createDataParser();
 * auto result = parser->parse("input data");
 * if (result) {
 *     // 使用解析结果
 * }
 * 
 ...
 * 
 * @throws std::invalid_argument 如果 inputText 为空
 * 
 * @note 解析器会尽可能容错，跳过无法识别的内容
 * 
 * @example
 * auto result = parser.Parse("input data content");
 */
Expected<ParsedData, ParseError> Parse(const std::string& inputText);
```
...
matrix[50][100] = 3.14;  // 直接下标访问

// 场景2：固定大小缓冲区 - 读取文件或网络数据
size_t buffer_size = 4096;
std::vector<uint8_t> buffer(buffer_size);  // 分配 4KB 缓冲区
file.read(reinterpret_cast<char*>(buffer.data()), buffer_size);

// 场景3：结果数组与输入等长 - 一对一映射转换
std::vector<int> input = GetInput();
std::vector<double> output(input.size());  // 输出与输入等长
for (size_t i = 0; i < input.size(); ++i) {
    output[i] = input[i] * 2.5;  // 直接赋值，无需 push_back
}

// 场景4：查找表/映射表 - 大小固定
std::vector<int> lookup_table(256, -1);  // ASCII 查找表，初始化为 -1
for (char c = 'a'; c <= 'z'; ++c) {
    lookup_table[static_cast<unsigned char>(c)] = c - 'a';  // 映射 a-z 到 0-25
}

// 场景5：算法中间结果 - 动态规划、计数等
int n = 1000;
std::vector<int> dp(n + 1, 0);  // DP 数组，初始化为 0
std::vector<int> count(26, 0);   // 字母计数数组
...
推荐在 CI/CD 流程中集成以下静态分析工具，实现自动化代码质量检查：

| 工具 | 用途 | 特点 |
|------|------|------|
| cppcheck | 通用静态分析 | 零误报率优先，检测常见错误 |
| clang-tidy | 代码风格+缺陷检测 | 基于 Clang，可自动修复 |
| PVS-Studio | 深度缺陷检测 | 商业工具，检测能力强 |
...
| 检查类别 | 说明 | 示例 |
|----------|------|------|
| `error` | 严重错误（默认开启） | 内存泄漏、空指针解引用 |
| `warning` | 潜在问题 | 未使用的变量、可疑的比较 |
| `style` | 代码风格问题 | 冗余代码、不一致的命名 |
| `performance` | 性能问题 | 不必要的拷贝、低效的容器使用 |
| `portability` | 可移植性问题 | 平台相关的类型转换 |
...
1. **提交前检查**：在 pre-commit hook 中运行快速检查
2. **PR 检查**：在 CI 流水线中运行完整分析
3. **定期扫描**：每周对整个代码库进行深度分析
4. **增量检查**：仅检查变更的文件，提高效率
...
| 工具类型 | 工具名称 | 用途 | 阶段 |
|----------|----------|------|------|
| 静态分析 | clang-tidy performance-* | 检测低效代码模式 | 编译期 |
| 内存检测 | Valgrind / ASan | 内存泄漏、越界访问 | 运行时 |
| 线程检测 | TSan | 数据竞争、死锁 | 运行时 |
| 性能剖析 | perf / VTune | CPU 热点分析 | 运行时 |
| 缓存分析 | Cachegrind | 缓存命中率分析 | 运行时 |
| 基准测试 | Google Benchmark | 性能回归检测 | 测试期 |

**1. Google Sanitizers（推荐集成到 CI）**：

```bash
# AddressSanitizer (ASan) - 内存错误检测
g++ -fsanitize=address -g -O1 main.cpp -o main_asan
./main_asan  # 运行时检测内存问题

# LeakSanitizer (LSan) - 内存泄漏检测（ASan 默认包含）
ASAN_OPTIONS=detect_leaks=1 ./main_asan

# ThreadSanitizer (TSan) - 线程问题检测
...
cg_annotate cachegrind.out.<pid>  # 查看分析结果

# 调用图分析
valgrind --tool=callgrind ./myprogram
kcachegrind callgrind.out.<pid>  # 可视化查看
```
...
**4. Google Benchmark 性能基准测试**：

```cpp
#include <benchmark/benchmark.h>

// 基准测试：vector push_back vs reserve + push_back
...
```bash
# 运行基准测试
./benchmark_test --benchmark_format=json > benchmark_result.json
...
| 检查类型 | 触发时机 | 说明 |
|----------|----------|------|
| ASan + UBSan | 每次 PR | 检测内存和未定义行为问题 |
| TSan | 每次 PR（多线程代码） | 检测数据竞争 |
| 基准测试 | 每次 PR | 检测性能回归（对比基准值） |
| Valgrind | 每周定期 | 深度内存分析（运行较慢） |
| perf 分析 | 发布前 | 热点分析和优化 |
...
| 工具类型 | 工具名称 | 用途 | 特点 |
|----------|----------|------|------|
| 静态分析 | Flawfinder | C/C++ 安全漏洞扫描 | 轻量快速，CWE 映射 |
| 静态分析 | clang-tidy security-* | 安全相关代码模式检测 | 集成到编译流程 |
| 静态分析 | Semgrep | 自定义安全规则匹配 | 规则易编写 |
| 动态分析 | ASan + UBSan | 内存越界、未定义行为 | 运行时检测 |
| 模糊测试 | libFuzzer | 自动化输入测试 | 与 ASan 配合使用 |
| 模糊测试 | AFL++ | 覆盖率引导模糊测试 | 高效发现漏洞 |

**1. Flawfinder - 安全漏洞静态扫描**：
...
# 输出 CSV 格式便于处理
flawfinder --csv src/ > vulnerabilities.csv
```
...
| 风险函数 | 安全问题 | 建议替代 |
|----------|----------|----------|
| `strcpy`, `strcat` | 缓冲区溢出 | `strncpy`, `strncat` 或 `std::string` |
| `sprintf` | 格式化字符串漏洞 | `snprintf` 或 `std::format` |
| `gets` | 无边界检查 | `fgets` 或 `std::getline` |
| `scanf("%s")` | 缓冲区溢出 | 指定宽度 `scanf("%99s")` |
| `rand` | 弱随机数 | `<random>` 或系统随机源 |

**2. clang-tidy 安全检查**：

```bash
# 启用安全相关检查
clang-tidy -checks='bugprone-*,cert-*,security-*' src/*.cpp -- -std=c++17

# 常用安全检查项
# bugprone-suspicious-string-compare  - 可疑的字符串比较
# cert-err34-c                        - 检测 atoi 等不安全函数
# cert-msc30-c                        - 检测不安全的随机数
# security-insecure-api-*             - 不安全 API 调用
```
...
Checks: >
  ...
  security-*,
  -cert-err58-cpp
WarningsAsErrors: 'security-*,cert-err34-c'
```

**3. libFuzzer 模糊测试**：

```cpp
// fuzz_target.cpp - 模糊测试目标函数
#include <cstdint>
#include <cstddef>
...
```bash
# 编译模糊测试目标（需要 clang）
clang++ -g -fsanitize=fuzzer,address -o fuzz_test fuzz_target.cpp parser.cpp

# 运行模糊测试
mkdir corpus  # 种子语料库目录
./fuzz_test corpus/ -max_total_time=3600  # 运行 1 小时
...
# 准备种子输入
mkdir -p testcases
echo "sample input" > testcases/seed1

# 运行模糊测试
afl-fuzz -i testcases -o findings ./my_program @@
```
...
```bash
#!/bin/bash
# security_check.sh - 综合安全检查脚本

echo "=== Running Flawfinder ==="
flawfinder --minlevel=2 --csv src/ > flawfinder_report.csv

echo "=== Running clang-tidy security checks ==="
clang-tidy -checks='bugprone-*,cert-*,security-*' \
    src/*.cpp -- -std=c++17 2>&1 | tee clang_tidy_security.log

echo "=== Checking for banned functions ==="
grep -rn --include="*.cpp" --include="*.h" \
    -E "(strcpy|strcat|sprintf|gets|scanf\s*\(\s*\"%s)" src/
...
if(ENABLE_FUZZING)
    if(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
        ...
        target_link_libraries(fuzz_parser myproject_lib)
    else()
...
| 检查类型 | 触发时机 | 说明 |
|----------|----------|------|
| Flawfinder | 每次 PR | 快速扫描已知危险模式 |
| clang-tidy security-* | 每次 PR | 深度安全代码检查 |
| ASan + UBSan | 每次 PR | 运行时安全检测 |
| 模糊测试 | 每日/每周 | 持续发现新漏洞 |
| 依赖漏洞扫描 | 每周 | 检查第三方库 CVE |

**安全漏洞分类（CWE）**：

| CWE ID | 漏洞类型 | 检测工具 |
|--------|----------|----------|
| CWE-120 | 缓冲区溢出 | Flawfinder, ASan |
| CWE-416 | 释放后使用 | ASan |
| CWE-476 | 空指针解引用 | UBSan, clang-tidy |
| CWE-190 | 整数溢出 | UBSan |
| CWE-362 | 竞态条件 | TSan |
| CWE-732 | 不安全权限 | 人工审查 |
